...
Build by hand lets a human name the workspace and add a note, checklist, or table without a WebMCP host. Every action enters the same command log the agent reads; a new table has no fields and explicitly asks the agent to bind them.
...
- **Proof.** Run `control-chameleon browser snapshot --aria --path artifacts/build-by-hand/board.aria.txt` and `control-chameleon browser screenshot --path artifacts/build-by-hand/board.png`. Both artifacts show the renamed board, all three new widget headings, the checklist item, the table handoff, and the latest human activity.
...
- Notes and checklists with default titles are distinct only by widget type. Rename the board does not rename widgets.

...
- **Drag note.** Drag the welcome note to the right. Run `control-chameleon browser drag --name "A canvas that listens" --dx 320 --dy 0 --wait-text "Latest: Moved “A canvas that listens”"`. The footer shows that summary and `state v2 · 2 commands`. Confirm the version with `control-chameleon browser assert --text "state v2 · 2 commands"`.
...
- **Resize note.** Pull the southeast handle. Run `control-chameleon browser resize --name "A canvas that listens" --dx 80 --dy 80 --wait-text "Latest: Resized “A canvas that listens”"`. The result reports `"changed": true`, larger before/after dimensions, and the footer shows that summary with `state v3 · 3 commands`.
...
- `browser resize` targets the visible grip in the handle's lower-right corner. The span's centre is covered by card content in headless Chrome and produces no mutation.

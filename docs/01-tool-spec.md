...
|---|---|
| `INVALID_INPUT` | "Your arguments failed schema validation. See details for the exact issues, fix them, and retry." |
| `INVALID_CONFIG` | "The config does not match this widget type's schema. details contains the expected schema for this type." |
...
| `WIDGET_NOT_FOUND` | "No widget has this id. Call describe_current_state to list valid widget ids — the human may have deleted it." |
| `WRONG_WIDGET_TYPE` | "This tool does not apply to this widget's type. describe_current_state shows each widget's type." |
| `ROW_NOT_FOUND` | "One or more rowIds do not exist in this widget. Call read_widget_data to get current row ids." |
| `NO_FIELDS_BOUND` | "This widget has no field schema yet. Call bind_data first to define its fields." |
| `FIELD_NOT_FOUND` | "Config references a field key that is not in the schema. details names the missing key." |
| `NAME_TAKEN` | "A tool with this name already exists. Pick a more specific name, or call remove_minted_tool first if you own it." |
| `RESERVED_NAME` | "This name is reserved by the app. Choose a different verb_noun name." |
...
| `LIMIT_EXCEEDED` | "A hard limit was hit (see details: which limit and the maximum). Remove something before adding more." |
